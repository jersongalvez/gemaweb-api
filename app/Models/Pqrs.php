...
class Pqrs extends Model
{
    ...
    public function search() {
        $user = DB::table('AFILIADOSSUB')
              ...
              ->where("AFILIADOSSUB.TIP_DOCUMENTO_BEN", 'CC')
              ->where("AFILIADOSSUB.NUM_DOCUMENTO_BEN", '1000032307')
              ->where("AFILIADOSSUB.EST_AFILIADO", 1)
              ->limit(1)
              //->where("a.FEC_EXPDOCUMENTO", '2973')
              ->get();
    ...
    public function searchPqrs() {
        $pqrs = DB::table("QUEJAS")
              ...
              ->select("QUEJAS.DETALLE", "QUEJAS.SOL_ESPERADA", "QUEJAS.PRI_NOMBRE", "QUEJAS.SEG_NOMBRE", "QUEJAS.PRI_APELLIDO", "QUEJAS.SEG_APELLIDO", "QUEJAS.CELULAR1", "QUEJAS.CORREOEMAIL", "QUEJAS.DIRECCION", "QUEJAS.FECHA_REGISTRO", "TRAMITE_QUEJAS.DES_SOLUCION", "ESTADOS_QUEJA.DES_EST_QUEJA")
              ->where("QUEJAS.TP_DOC_AFI", "TI")
              ->where("QUEJAS.NM_DOC_AFI", "1030287427")
              ->where("QUEJAS.CONSECUTIVO", "6923")
    ...
    public function create($datos) {
       DB::table('QUEJAS')->insert([
         ...
         "URL_DOCUMENTOS" => $datos["tpdocumento"],
